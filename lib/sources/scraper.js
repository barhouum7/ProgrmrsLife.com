...
export async function fetchFromApify() {
    ...
    for (const actorId of actorIds) {
        try {
            ...
            // Define input — different actors may use different input schemas
            // IMPORTANT: Wrap URLs in quotes for exact phrase matching!
            // Without quotes, Twitter search matches individual words ("join" is too common)
            // Use since: operator to only get recent tweets (more likely to still be valid)
            const since48h = new Date(Date.now() - 48 * 60 * 60 * 1000)
                .toISOString().slice(0, 10); // YYYY-MM-DD
            const searchQuery = `"canva.com/brand/join" since:${since48h}`;
            ...
            const input = {
                ...
                queries: [searchQuery],
                startUrls: [
                    { url: `https://x.com/search?q=%22canva.com%2Fbrand%2Fjoin%22+since%3A${since48h}&f=live` } // Example: https://x.com/search?q=%22canva.com%2Fbrand%2Fjoin%22+since%3A2026-08-12&f=live
                ],
                maxTweets: 25,
                maxItems: 25,
                sort: 'Latest',
            ...
            const runData = await actorClient.call(input, { waitSecs: 90 });

            // Fetch results from the default dataset
            const datasetClient = client.dataset(runData.defaultDatasetId);
            const { items } = await datasetClient.listItems();

            console.log(`[Apify] Actor ${actorId} returned ${items.length} raw items.`);

            if (items.length === 0) continue;

            // Extract Canva URLs from items
            const links = extractLinksFromTweets(items, 'apify');

            console.log(`[Apify] Extracted ${links.length} Canva links from ${items.length} tweets.`);

            if (links.length > 0) return links;

            // If strict extraction found 0 links, log a sample for debugging
            if (items.length > 0 && links.length === 0) {
                const sample = items[0];
                console.log(`[Apify] Sample tweet text: "${(sample.full_text || sample.text || '').substring(0, 200)}"`);
                const sampleEntities = sample.entities?.urls || sample.entities || [];
                console.log(`[Apify] Sample entities:`, JSON.stringify(sampleEntities).substring(0, 300));
            }

            // Even if no strict Canva links found, continue to next actor
            continue;

        } catch (error) {
            const msg = error.message || String(error);
            console.error(`[Apify] Actor ${actorId} failed: ${msg}`);
            continue; // Try next actor
        }
    }

    console.log('[Apify] All actors exhausted, returning empty.');
    return [];
}
