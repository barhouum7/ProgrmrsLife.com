...
export async function fetchFromApify() {
    ...
    for (const actorId of actorIds) {
        const passes = buildQueryPasses();
        for (const pass of passes) {
            try {
                console.log(`[Apify] Actor: ${actorId} | Pass: ${pass.label} | Query: ${pass.query}`);
                ...
                console.log(`[Apify] Pass ${pass.label}: ${items.length} raw items.`);

                if (items.length === 0) continue;

                // Extract Canva URLs from items
                const links = extractLinksFromTweets(items, 'apify');
                console.log(`[Apify] Pass ${pass.label}: extracted ${links.length} Canva links.`);

                if (links.length > 0) {
                    return links; // Got at least something — caller decides if they’re new
                }

                // Log a sample for debugging when extraction fails
                const sample = items[0];
                console.log(`[Apify] Sample text: "${(sample.full_text || sample.text || '').substring(0, 200)}"`);
                ...
                console.error(`[Apify] Actor ${actorId} / Pass ${pass.label} failed: ${msg}`);
                // Continue to next pass, then next actor
            }
        }
    }

    console.log('[Apify] All actors and passes exhausted, returning empty.');
    return [];
}
