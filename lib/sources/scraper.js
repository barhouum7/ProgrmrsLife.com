    ...
    for (const actorId of actorIds) {
        try {
            ...
            const webhookUrl = callbackBaseUrl
                ? `${callbackBaseUrl}/api/scrape/apify-callback?secret=${encodeURIComponent(process.env.APIFY_WEBHOOK_SECRET || '')}`
                : null;
            ...
            return run.id; // Return runId string for polling
...
/**
 * Poll Apify for the status of a specific run and harvest results if done.
 * Returns:
 *   null    — run still in progress (RUNNING / READY)
 *   []      — run failed / aborted
 *   Link[]  — run SUCCEEDED, links extracted
 */
export async function fetchApifyRunStatus(runId) {
    const token = process.env.APIFY_API_TOKEN;
    ...
    try {
        ...
        console.log(`[Apify] Polling run ${runId}: ${status}`);

        if (status === 'RUNNING' || status === 'READY' || status === 'TIMING-OUT') return null;
        if (status !== 'SUCCEEDED') {
            console.log(`[Apify] Run ${runId} ended with ${status} — no results.`);
            return []; // Failed/aborted — signal completion with no links
        }
        return await fetchDatasetFromApifyRun(defaultDatasetId);
    } catch (err) {
        console.error(`[Apify] fetchApifyRunStatus error for ${runId}: ${err.message}`);
        return null;
    }
}
