    ...
    try {
        ...
        for (const sourceType of enabledSources) {
            try {
                ...
                switch (sourceType) {
                    case 'scraper_apify': {
                        // Fire-and-forget: start actor, results arrive via webhook callback
                        const started = await fetchFromApify(baseUrl);
                        if (started) {
                            console.log('[Scraper] scraper_apify: actor started async (webhook will deliver results).');
                        ...
                        // Always continue to Sorsa as a synchronous safety net
                        // (its response is fast enough to stay under Vercel’s 10s limit)
                        continue;
                    }
                    case 'scraper_sorsa':
