...
import { fetchFromApify, fetchFromSorsa, fetchApifyRunStatus } from './scraper';
...
// Pending Apify run ID — polled on next fetchCanvaLinks call instead of relying on webhooks
let _pendingApifyRunId = null;
 ...
 * @param {boolean} forceFetch - bypass cache
 * @param {object|null} req - Next.js request object (used to build Apify webhook URL)
 */
export async function fetchCanvaLinks(forceFetch = false, req = null) {
    const results = {
        links: [],
        sources: [],
        cached: false,
    ...
    try {
        ...
        if (cachedLinks.length > 0 && !forceFetch) {
            ...
            if (shouldScrape) {
                ...
                const baseUrl = req ? `${req.headers['x-forwarded-proto'] || 'https'}://${req.headers.host}` : null;
                runScrapersInBackground(baseUrl).catch(err => {
                    console.error('[Background Scrape] Error:', err.message);
                });
            }

            // Poll for pending Apify run results (replaces webhook — works even after cold starts)
            if (_pendingApifyRunId && !_scraping) {
                harvestPendingApifyRun().catch(err => {
...
/**
 * Check if a pending Apify run has completed and save its results.
 * Called on each fetchCanvaLinks invocation as a lightweight poll.
 */
async function harvestPendingApifyRun() {
    if (!_pendingApifyRunId) return;
    const runId = _pendingApifyRunId;
    const links = await fetchApifyRunStatus(runId);

    if (links === null) {
        console.log(`[Apify Poll] Run ${runId} still in progress.`);
        return; // Not done yet
    }

    // Run is done (succeeded or failed) — clear the pending ID
    _pendingApifyRunId = null;

    if (links.length === 0) {
        console.log(`[Apify Poll] Run ${runId} produced no new Canva links.`);
        await expireDownvotedScraperLinks().catch(() => { });
        return;
    }

    // Save new links
    const newCount = await saveLinksToDb(links);
    console.log(`[Apify Poll] Run ${runId} saved ${newCount} new links.`);
    if (newCount > 0) {
        await setLastScrapeTime(Date.now(), true);
        await cleanupOldLinks().catch(() => { });
    }
}
 ...
 */
async function runScrapersInBackground(baseUrl = null) {
    // shouldTriggerScrape already validated the cooldown — bypass the guard inside runScrapers
    await runScrapers(baseUrl, true);
 ...
 * @param {string|null} baseUrl - base URL for Apify webhook callback
 * @param {boolean} bypassCooldown - skip the time-based guard (caller already checked shouldTriggerScrape)
 */
async function runScrapers(baseUrl = null, bypassCooldown = false) {
    ...
    // Only enforce the time guard when the caller hasn’t already validated the cooldown
    if (!bypassCooldown) {
        const now = Date.now();
        const lastScrape = await getLastScrapeTime();
        const cooldown = _lastScrapeFoundLinks ? MIN_SCRAPE_INTERVAL_MS : 30 * 60 * 1000;
        if (now - lastScrape < cooldown) {
            const mins = Math.round((now - lastScrape) / 60000);
            const waitMins = Math.round((cooldown - (now - lastScrape)) / 60000);
            console.log(`[Scraper] Too soon since last scrape, skipping. (${mins}min ago, wait ~${waitMins}min)`);
            return [];
        }
    }
    ...
    try {
        ...
        for (const sourceType of enabledSources) {
            try {
                ...
                switch (sourceType) {
                    case 'scraper_apify': {
                        // Fire-and-forget: start actor. Store runId for polling on next request.
                        const runId = await fetchFromApify(baseUrl);
                        if (runId && typeof runId === 'string') {
                            _pendingApifyRunId = runId;
                            console.log(`[Scraper] scraper_apify: actor started async, runId: ${runId}`);
                            await prisma.sourceConfig.update({
                                where: { sourceType },
                                data: { lastRunAt: new Date(), lastRunSuccess: true },
                            }).catch(() => { });
                        } else if (runId === true) {
                            // Fallback: fetchFromApify returned bool true (old behavior)
                            console.log('[Scraper] scraper_apify: actor started (no runId returned).');
                        } else {
                            console.log('[Scraper] scraper_apify: failed to start actor, falling through to next provider.');
                        }
                        // Always also run Sorsa synchronously as an immediate fallback
                        continue;
                    }
                    case 'scraper_sorsa':
