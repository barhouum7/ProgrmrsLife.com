    ...
    try {
        ...
        if (isMockData) {
            ...
            try {
                ...
                // Try to log the failed API call (don't let this crash the handler)
                try {
                    ...
                    console.error('Failed to log API call:', logError.message);
                }

                // For 429 (rate limited) or 403 (access revoked), fall back gracefully
                if (error.response?.status === 429 || error.response?.status === 403) {
                    ...
                    return { 
                        ...
                        error: error.response?.status === 403 
                            ? { message: 'Twitter API access has been revoked. Showing cached data.' }
                            : null
                    };
                }
                throw error;
