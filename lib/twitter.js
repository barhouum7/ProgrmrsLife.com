...
const BEARER_TOKEN = process.env.TWITTER_2_BEARER_TOKEN;
        ...
        if (processedTweets?.length > 0) {
            try {
                ...
                await executeWithRetry(async () => {
                    ...
                    // Deactivate votes for tweets that no longer exist
                    const existingTweetIds = processedTweets.map(t => t.id);
                    const deleteVotesResult = await prisma.vote.updateMany({
                        where: {
                            tweetId: {
                    ...
                    console.log(`Deactivated ${deleteVotesResult.count} votes for non-existent tweets`);
                });
